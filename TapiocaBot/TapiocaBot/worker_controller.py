...
import random
from worker_scout_controller import WorkerScoutController
from worker_militia_controller import WorkerMilitiaController
...
class WorkerController:
    def __init__(self, bot=None, verbose=False):
        ...
        self.scouting_controller = WorkerScoutController(
            bot=self.bot,
            verbose=self.verbose
        )

        self.militia_controller = WorkerMilitiaController(
            bot=self.bot,
            verbose=self.verbose
        )

    async def step(self):
        await self.militia_controller.step()
        await self.scouting_controller.step()
    ...
    async def handle_idle_workers(self):
        ...
        idle_workers = self.bot.units(UnitTypeId.PROBE).idle.filter(
            lambda unit: unit.tag not in self.scouting_controller.scouting_workers
        )
